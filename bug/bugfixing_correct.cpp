...
public:
    ...
    ~Payload()
    { // Automatically unmount the payload when it goes out of scope
        if (mounted)
            unmount();
    }
};
...
public:
    explicit Rover(std::string name, std::vector<Camera> cameras = {}, std::vector<Rock> rocks = {}, int photosTaken = 0)
        : name(std::move(name)), cameras(std::move(cameras)), rocks(std::move(rocks)), photosTaken(photosTaken) {}
    ...
    {
        if (this->payload == nullptr)
            throw std::logic_error("No payload mounted");
    ...
    {
        if (cameras.empty())
            throw std::logic_error("No camera mounted");
...
public:
    ...
    static Rock collectRock()
    {
        ...
        return Rock(10e3 * dist(gen), dist(gen), dist(gen), dist(gen));
    }
};
...
int main()
{
    ...
    rover.shootPhoto();
    rover.shootPhoto();
    assert(rover.photosTakenCount() == 3);
